...
double complex** soma(double complex** mtx_a, double complex** mtx_b, int linhas, int colunas)
{
	...
	for (int l = 0; l < linhas; l++)
	{
		for (int c = 0; c < colunas; c++)
		{
			matriz[l][c] = mtx_a[l][c] + mtx_b[l][c];
		}
	}
	return matriz;
}
...
double complex** allocateComplexMatrix (int linhas, int colunas)
{
	...
	matrix = (double complex **) malloc(linhas*sizeof(double complex*));
	...
	// Memory allocation for each row of the matrix
	for (int i = 0; i < linhas; i++)
	{
		matrix[i] = (double complex *) malloc(colunas*sizeof(double complex));
		if (matrix[i] == NULL)
		{
			printf("Memory allocation failed\n");
			exit(1);
